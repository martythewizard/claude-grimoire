...
![Agent Teams vs Subagents](AgentTeamsVsSubagents.png)
...
- **tmux required**: Agent Teams visualization requires tmux. Native Windows (without WSL) is not supported.
- **Env var must be set inside tmux**: If you `export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in your terminal, it won't carry into a new tmux session. Use `~/.claude/settings.json` instead (see [Enable Agent Teams](#2-enable-agent-teams)).
- **Agents share the filesystem**: Two agents editing the same file will conflict. Assign strict file ownership in agent prompts.
- **Higher token cost**: Each agent is a full Claude session. Budget accordingly for complex builds.
- **Experimental feature**: Agent Teams behavior may change between Claude Code releases.
