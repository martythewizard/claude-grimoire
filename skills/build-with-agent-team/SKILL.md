...
If team size is specified (`$2`), use that number of agents.
...
Enable tmux split panes so each agent is visible. Pass this mode when creating the team (via `TeamCreate` or equivalent):

```json
{ "teammateMode": "tmux" }
```

This opens each agent in a separate tmux pane so you can watch all agents work simultaneously. Requires tmux installed and the `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` env var set (see prerequisites).
